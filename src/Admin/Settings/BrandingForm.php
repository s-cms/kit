...
class BrandingForm
{
    public static function make(): Tab
    {
        $icons = collect(LucideIcon::cases())->mapWithKeys(function (LucideIcon $icon): array {
            ...
            return [
                $icon->value => "<div style='display: flex; gap: 10px; align-items: center;'> $iconHtml <span class='text-sm'>{$icon->name}</span></div>",
            ];
        });

        return Tab::make(__('kit::admin.branding'))->schema([
            TextInput::make('branding.twitter_name')->label(__('kit::admin.twitter_name'))->string(),
            Repeater::make('branding.socials')
                ->schema([
                    TextInput::make('name')->label(__('support::admin.name'))->string()->required(),
                    TextInput::make('link')->label(__('kit::admin.url'))->string()->required(),
                    Grid::make(2)->schema([
                        Select::make('icon')
                            ->options($icons)
                            ->allowHtml()
